...
> 2026-08-13 更新：歌曲页已收敛为 `index.html`，视频补录与别称维护已收敛为 `editor.html`；歌曲页、视频页与编辑器的样式和脚本已全部拆分到 `assets/`。视频页在手机端改为信息完整的响应式卡片，歌曲页与视频页都新增了完整筛选抽屉；搜索、筛选和排序现可通过网址保存、分享、刷新恢复及历史回退。`npm run check:browser` 已覆盖这些核心流程。三个旧入口保留兼容跳转。数据更新流程已增加事务化发布、深层校验、异常缩量拦截与最新视频增量富化。

> 2026-08-13 Staff 复核更新：统一编辑器新增 `?tab=staff` 工作台，可把 `staff_review.json` 展开为逐行问题队列，生成角色别名、人名别名和整行忽略规则。解析器支持 `ignore` 角色和精确忽略行，相关手工文件变化会触发自动重建。

## 2026-08-13 本轮验收状态

- 歌曲页加载 712 首，别称“洛基”命中 `ロキ`；搜索与排序状态可刷新恢复。
- 视频页加载 434 条，`CRASH THE PARTY` 搜索得到 2 条。
- 390px 手机视口可打开完整筛选抽屉；选择“官方 2DMV”后得到 266 条，计数、网址和历史返回同步正确。
- 手机视频卡片保留版本、频道、播放量和日期四项信息，可展开详情，页面宽度 390 / 390，无横向溢出。
- 前端共享控制逻辑集中在 `assets/viewer_controls.js` 与 `assets/viewer_controls.css`，歌曲页和视频页使用同一套抽屉、焦点与网址状态实现。
- Staff 复核页加载 223 条审计视频记录、456 条未知角色行和 661 条未解析行；真实浏览器已验证角色映射、整行忽略、标签切换保留草稿及 390px 单列操作。
